 ...
 * @author Quintin
 */
public class Spark extends CANSparkMax implements Controller, Loggable {
	...
	static class SparkController extends ProfiledControllerBase<Spark> {
		...
		protected void updateGains(boolean isInit, int slot, Gains newGains, Gains lastGains) {
			...
			if (isInit) {
				mPidController.setSmartMotionAccelStrategy(SparkMaxPIDController.AccelStrategy.kSCurve, slot);
			}
		}
		...
		protected boolean setReference(ControllerOutput.Mode mode, int slot, double reference, double arbitraryPercentOutput) {
			...
			return mPidController.setReference(reference, controllerType, slot, arbitraryPercentOutput,
					ArbFFUnits.kPercentOut) == REVLibError.kOk;
		}
	...
	@Override
	public void log() {
		CSVWriter.addData(getName() + " Current", getOutputCurrent());
		CSVWriter.addData(getName() + " PO", getAppliedOutput());
		CSVWriter.addData(getName() + " Temp", getMotorTemperature());
	}
