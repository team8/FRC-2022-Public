...
public class RoutineManager {
	...
	static Set<Class<? extends SubsystemBase>> sharedSubsystems(List<RoutineBase> routines) {
		Set<Class<? extends SubsystemBase>> sharedSubsystems = new HashSet<>(); // TODO: No allocation on update
	...
	public void update(Commands commands, @ReadOnly RobotState state) {
		...
		for (int i = mRunningRoutines.size() - 1; i >= 0; i--) {
			var runningRoutine = mRunningRoutines.get(i);
			if (commands.wantedCancels.contains(runningRoutine.getClass())) {
				runningRoutine.stop(commands, state);
				mRunningRoutines.remove(runningRoutine);
			}
		}
