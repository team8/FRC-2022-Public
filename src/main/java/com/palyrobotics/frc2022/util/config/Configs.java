...
public class Configs {
	...
	private static final String kConfigFolderName = "config";
	private static final String kLoggerTag = Util.classToJsonName(Configs.class);
	...
	private static final ObjectWriter sPrettyWriter = sMapper.writerWithDefaultPrettyPrinter();
	private static final Thread sModifiedListener = new Thread(Configs::watchService);
	private static final Thread sRobotThread = Thread.currentThread();
	...
	public static <T extends ConfigBase> T get(Class<T> configClass, String name) {
		...
		try {
			...
			throw new ConfigException(parseException, configClass);
		}
	}
	...
	static Optional<String> getDefaultJson(Class<? extends ConfigBase> configClass) {
		try {
	...
	private static <T extends ConfigBase> T read(Class<T> configClass) {
		...
		try {
			...
			var exception = new ConfigException(readException, configClass);
	...
	public static <T extends ConfigBase> boolean save(Class<T> configClass) {
		try {
			...
			Log.error(kLoggerTag, String.format("Could not save config %s", configClass.getSimpleName()), saveException);
			return false;
		}
	}
	...
	public static boolean reload(Class<? extends ConfigBase> configClass) {
		ConfigBase existing = sConfigMap.get(configClass);
		ConfigBase onFile = read(configClass);
		try {
	...
	private static void doPoll(List<String> alreadySeen, WatchEvent<?> pollEvent) {
		if (pollEvent.kind() == StandardWatchEventKinds.OVERFLOW) return;
